...
class Browser : public QTextBrowser {
  ...
  protected:
   void contextMenuEvent(QContextMenuEvent *ev) {
      ...
      qDebug() << "position in block" << cur.positionInBlock() << "object type"
               << cur.charFormat().objectType();
      if (fmt.objectType() == QTextFormat::NoObject) {
         // workaround, sometimes the cursor will point one object to the left of the
         // image
