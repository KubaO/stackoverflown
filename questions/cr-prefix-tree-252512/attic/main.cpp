...
public:
    ...
    NodeIndex(char ch, unsigned int index) : m_index(((unsigned int) ch) << 24 | index)
    {
        assert(index <= 0xFFFFFF);
    }
    inline char ch() const { return (m_index >> 24) & 0xFF; }
    inline unsigned int index() const { return m_index & 0xFFFFFF; }
    bool operator<(const NodeIndex &o) const { return ch() < o.ch();  }
};
...
struct Node
{
    ...
    NodeIndex nodes[1];
    unsigned int getIndex(char ch) const
    {
        auto n = std::lower_bound(nodes + 0, nodes + childCount, ch);
        if (n == (nodes + childCount) || n->ch() != ch)
            return 0;
        return n->index();
    }
...
static_assert(sizeof(decltype(pool)::value_type) == sizeof(NodeIndex), "");

unsigned int make_node(unsigned char childCount)
{
    unsigned int index = pool.size();
    auto const size = (sizeof(Node) + 1) / sizeof(NodeIndex) + childCount - 1;
